    ...
    if SKILLS_DIR.exists():
        for p in sorted(SKILLS_DIR.iterdir()):
            if p.is_dir() and (p / "SKILL.md").exists():
                try:
                    content = (p / "SKILL.md").read_text(encoding="utf-8", errors="replace")[:500]
                except Exception:
                    content = ""
                skills.append({
                    "name": p.name,
                    "path": str(p.relative_to(Path(HERMES_HOME))),
                    "preview": content,
...
async def api_mcp_get(request: Request):
    ...
    try:
        ...
        return JSONResponse({"mcp_servers": cfg.get("mcp_servers") or {}})
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=500)
...
async def api_cron_list(request: Request):
    ...
    jobs = []
    if CRON_DIR.exists():
        for p in sorted(CRON_DIR.glob("*.json")) + sorted(CRON_DIR.glob("*.yaml")) + sorted(CRON_DIR.glob("*.yml")):
            try:
                ...
                pass
    # Also check for any cron state files
    return JSONResponse({"jobs": jobs})


async def api_workspace_list(request: Request):
    if err := guard(request): return err
    path_param = request.query_params.get("path", "").strip().lstrip("/")
    if ".." in path_param:
        return JSONResponse({"error": "Invalid path"}, status_code=400)
    base = WORKSPACE_DIR
    target = (base / path_param).resolve() if path_param else base.resolve()
    try:
        target.relative_to(base.resolve())
    except ValueError:
        return JSONResponse({"error": "Path outside workspace"}, status_code=400)
    if not target.exists():
        return JSONResponse({"entries": [], "path": path_param})
    if target.is_file():
        try:
    ...
    for p in sorted(target.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower())):
        try:
            ...
            pass
    return JSONResponse({"type": "dir", "path": path_param, "entries": entries})
...
routes = [
    ...
    Route("/setup/api/skills",                  api_skills_list),
    Route("/setup/api/skills/get",              api_skills_get),
    Route("/setup/api/skills/delete",           api_skills_delete,   methods=["POST"]),
    Route("/setup/api/mcp",                     api_mcp_get),
    Route("/setup/api/mcp",                     api_mcp_put,         methods=["PUT"]),
    Route("/setup/api/cron",                    api_cron_list),
    Route("/setup/api/workspace",               api_workspace_list),
